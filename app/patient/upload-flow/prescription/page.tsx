...
import { generatePrescriptionData } from "@/app/api/gemini-ocr/route" // Import the server action
...
export default function PrescriptionUploadFlow() {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      setStep("capture") // Stay on capture if camera fails
    }
  }, [])
  ...
  const handleCapture = async () => {
    ...
    if (context) {
      ...
      try {
        // Call the server action with the image data
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="flex items-center p-4">
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-gray-900">Upload Prescription</h1>
            <p className="text-sm text-gray-600">Scan or upload your prescription</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        {step === "capture" && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <Button onClick={handleCapture} className="w-full bg-green-600 hover:bg-green-700 h-12">
        ...
        {step === "processing" && (
          <div className="text-center space-y-6 py-12">
            <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center mx-auto animate-spin">
              <Loader2 className="h-10 w-10 text-blue-600" />
            </div>
            <h2 className="text-xl font-bold text-gray-900">Processing Prescription...</h2>
            <p className="text-gray-600">Our AI is reading your document. This may take a moment.</p>
          </div>
        )}
        ...
        {step === "review" && (
          <div className="space-y-6">
                  ...
                  {detectedMedicines.map((med, index) => (
                    <div key={index} className="pb-4 border-b last:border-b-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-bold text-lg text-orange-600">{med.name}</h3>
                        <span className="font-semibold text-orange-600">{med.estPrice}</span>
                      </div>
            ...
            <Button onClick={handleViewOnMap} className="w-full bg-green-600 hover:bg-green-700 h-12">
