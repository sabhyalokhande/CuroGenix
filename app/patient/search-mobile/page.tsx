    ...
    if (medicinesFromPrescription.length > 0) {
      const filtered = allPharmacies
        .map((pharmacy) => {
          ...
          // Sort by how many medicines are available (descending), then by distance
          return {
            ...pharmacy,
            availableCount,
            totalCount,
            sortScore: availableCount * 1000 - Number.parseFloat(pharmacy.distance), // Higher availableCount first, then lower distance
        ...
        .sort((a, b) => b.sortScore - a.sortScore) // Sort by sortScore
    ...
    <div className="min-h-screen bg-gray-50 max-w-sm mx-auto">
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="flex items-center p-4">
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-gray-900">Find Medicine</h1>
            {medicinesFromPrescription.length > 0 ? (
              <p className="text-sm text-gray-600">Searching for: {medicinesFromPrescription.join(", ")}</p>
            ) : (
              <p className="text-sm text-gray-600">Search all medicines</p>
            )}
          </div>
          <Button variant="ghost" size="sm" onClick={() => setShowMap(!showMap)} className="p-2">
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2 mt-2">
                      {["1 km", "2 km", "5 km", "10 km"].map((dist) => (
                        <Button key={dist} variant="outline" size="sm" className="flex-1 bg-transparent">
                          {dist}
                        </Button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2 mt-2">
                      {["All", "In Stock", "Low Stock"].map((status) => (
                        <Button key={status} variant="outline" size="sm" className="flex-1 bg-transparent">
                          {status}
                        </Button>
                      ))}
                    </div>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-medium text-gray-900">Results ({filteredPharmacies.length})</h2>
        ...
        <div className="space-y-3">
          {filteredPharmacies.map((pharmacy) => (
            <Card key={pharmacy.id} className="border-0 shadow-sm">
                    ...
                    {pharmacy.status !== "unavailable" && (
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <span className="text-sm text-gray-500 ml-2">Avg: {pharmacy.avgPrice}</span>
                        </div>
